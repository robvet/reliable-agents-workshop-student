...
class CrewAgent(BaseDomainAgent):
    """Finds crews and their assignments/ETA via the NL-2-SQL MCP endpoint.

    Speaks in domain terms only (crews, assignments, outages, ETA, location). It
    does not know table or column names - the NL-2-SQL layer behind the MCP
    boundary owns the schema and writes the SQL.
    """

    name = "crew"
    description = "Finds crews and their assignments/ETA. Needs events when scoping crews to specific outages."
    ...
    async def handle(self, request: AgentRequest) -> AgentResult:
        ...
        logging.info("CrewAgent: reasoning (question)=%s", prompt)
        try:
            ...
            logging.exception("CrewAgent: MCP query failed; failing the step")
            raise
        ...
        logging.info("CrewAgent: rows=%d sql=%s", data["count"], sql)
        return self._result(
            data=data,
            ok=True,
            summary=f"located crews ({self._describe(request)}); {data['count']} found",
            detail={
                "question": prompt,
                "tool": "ask",
                "sql": sql or "",
    ...
    def _build_prompt(self, request: AgentRequest) -> str:
        ...
        # Scope from the user's extracted entities (event/asset/location).
        if e.event_id:
        ...
        question = f"Answer the crew portion of this request: {request.user_prompt}"
        if filters:
    ...
    def _parse(self, payload) -> dict:
        ...
        if isinstance(payload, list):
            ...
            crews = payload.get("rows") or payload.get("crews") or []
        else:
            crews = []
