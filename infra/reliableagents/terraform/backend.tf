resource "azurerm_container_app" "backend" {
  ...
  # Direct PostgreSQL connection — used only by the admin data-generator feature.
  secret {
    name  = "database-url"
  ...
  template {
    ...
    container {
      name   = "backend"
      image  = "${azurerm_container_registry.main.login_server}/backend:v1"
      cpu    = 0.5
      memory = "1Gi"
  ...
  # CI/CD (deploy-backend.yml) updates the image to a git-SHA tag via
